...
function findEmployeeBySpecialization(specialization) {
    const foundEmployee = employees.filter(employee => employee.specialization === specialization);
    const foundEmployeeDisplay = foundEmployee.map((employee, index) => `<p>${employee.id}: ${employee.name}: ${employee.name} - ${employee.department} - $${employee.salary}</p>`).join('');
